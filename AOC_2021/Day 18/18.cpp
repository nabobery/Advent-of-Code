...
int part2(string file){
    ...
    while(getline(in, str)){
        numbers.push_back(parse(str));
    }
    int result = INT_MIN;
    for(int i=0; i<numbers.size() - 1; i++){
        vector <char> curr = numbers[i];
        for(int j= i+1; j<numbers.size(); j++){
            vector <char> temp = addSnailNumbers(curr, numbers[j]);
            reduce(temp);
            int curr_mag = magnitude(temp,0).first;
            result = max(result, curr_mag);
            temp = addSnailNumbers(numbers[j], curr);
            reduce(temp);
            curr_mag = magnitude(temp,0).first;
            result = max(result, curr_mag);
        }
    }
    return result;
...
int main(){
    ...
    cout<<"Part 1: "<< part1(file)<<"\n";
    cout<<"Part 2: "<< part2(file)<<"\n";
    return 0;
}
